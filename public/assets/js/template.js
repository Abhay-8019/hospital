...
function reportPrint(id, cssPath) {
    ...
    var print = '<html><head>'+css+'<style>@page { size: A4 portrait; margin: 3mm 0 -2mm 1mm;} h1,h2,h3,.padding0 {margin: 0;padding: 0;} hr {margin: 5px 0;}.h-size {font-size: 16px;font-weight: bold;}table { clear: both;font-size:12px; } body {background: #fff !important;} table tr th.print_hide, table tr td.print_hide { display: none; } table tr th {font-weight:normal;padding: 0;} table tr td {padding: 0;}</style></head><body>'+data+'</body></html>'
 ...
 * @param birthDate
 */
function calculateAge(birthDate, route, input)
{
    ...
    $.ajax({
        type: "POST",
        url: route,
        data: {'_token' : token, 'birthDate': birthDate},
        beforeSend: function() {
            $(".backDrop").fadeIn( 100, "linear" );
            $(".loader").fadeIn( 100, "linear" );
        },
 ...
 *  original Name of function is -> calcInches
 */

function calcHeightInches()
{
    var feet = Number($("#height_feet").val());
    var inches = Number($("#height_inches").val());

    return (Math.round(feet*12) + inches);
    //$('#convInches').val(roundit(feet*12) + inches);
}
/**
 * @returns ExcessWeight {*}
 * original Name of function is -> ExcessWeight
 */
function calcExcessWeight()
{
    var calcHeightInches = Number($("#calcHeightInches").val());
    var weight = Number($("#weight").val());
//    console.log(calcHeightInches + "-" + weight);
    var ideal_weight = ((calcHeightInches - 60) * (2.3)) + 50;
    var excessWeight = (weight - ideal_weight).toFixed(2);
    return excessWeight;
}
...
$('body').on('click', 'a.dEdit', function(event) { //due to ajax data request with datatable grids
        ...
        $.ajax({
            ...
            success:function(data, textStatus, jqXHR)
            {
                ...
                $("#dynamicEdit").find("#dataResult").find('.select2').select2({
                    allowClear: true
                });
    ...
    history.back(1);
}).on('click', '.__route', function(event) {
...
// add auto complete to inputs.
$( ":input[type=text]" ).attr('autocomplete','off');

$('body').on('change', 'select.ajaxChange', function(event)
...
$(function() {
    ...
    $(".normal-date-picker").datepicker({
        ...
        changeMonth: true,
        changeYear: true,
        yearRange: '1960:' + (new Date().getFullYear() + eval(2)),
    ...
    /*$(".select2-ajax").select2({
        ...
        //placeholder: "Please enter one charater",
                    ...
                    if (data.status == 206)
                    {
                        $.each(data.errors, function (i, value)
                        {
                            ...
                            if(split[1]) {
                                ...
                                $(document).find("input[name='"+ind+"']").parent().append('<div class="error">'+value+'</div>');
                                $(document).find("textarea[name='"+ind+"']").parent().append('<div class="error">'+value+'</div>');
                                $(document).find("select[name='"+ind+"']").parent().append('<div class="error">'+value+'</div>');
                            } else {
                                $(document).find("input[id="+i+"]").parent().append('<div class="error">'+value+'</div>');
                                $(document).find("textarea[id="+i+"]").parent().append('<div class="error">'+value+'</div>');
                                $(document).find("select[id="+i+"]").parent().append('<div class="error">'+value+'</div>');
                            }
                        });
