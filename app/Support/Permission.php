...
function getNavigation()
{
    ...
    $menuData = (new UserPermissions)->getPermissionsMenu($menuId, ['user_id'=> $userId]);
...
{
    $userId = authUser()->id;
    $result = (new RolePermissions)->getRolePermissions(['role_id'=> $userId], true);
    $menuId = (!empty($result)) ? $result['menu_id'] : '';

    $currentRouteName       = \Route::currentRouteName(); // current route name 
    $menuData = (new RolePermissions)->getPermissionsMenu($menuId, ['role_id'=> $userId]);
//    $routes                  = array_column($menuData, 'dependent_routes', 'route');
    $routes = [];
    foreach($menuData as $key => $val) {
        if($val['route'] != '') {
            $routes[$val['route']] = $val['dependent_routes'];
        }
    }
