...
class Menu extends Model
{
     ...
     * @return \Illuminate\Database\Eloquent\Builder
     */
    public function scopeNotNullMenu($query)
    {
        return $query->whereNotNull('menu.route');
    }
    /**
     * Scope a query to only include active Menu.
     *
    ...
    /**
     * @param bool $isMenu
     * @param bool $isTopMenu
     * @return array
     */
    public function getMenuNavigation($isMenu = false, $isTopMenu = false, $isSuperAdmin = false)
    {
        $userAssignedMenus = (new RolePermissions)->where('role_id', authUser()->role_id)->first();
        $query = $this->where('menu.status', 1);
        if($isTopMenu) {
            $query->where('menu.is_in_menu', 1);
        }
